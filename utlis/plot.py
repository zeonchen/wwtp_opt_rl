...
def action_plot(data, values, legend=False, ylabel='', xaxis='Epoch', condition=None):
    ...
    ax1.set_ylabel('Wastage rate ($m^3/d$)')

    ax2 = ax1.twinx()
    ax2_major_locator = MultipleLocator(2)
    ...
    if legend:
        ax1.legend(['Wastage rate'], loc='best')
    ...
    plt.savefig(fname="parameter.pdf", dpi=300, format="pdf")
    plt.show()
...
def reward_plot(data, values, ylabel='', xaxis='Epoch', condition=None):
    ...
    plt.tight_layout(pad=0.5)
    plt.savefig(fname="reward.pdf", dpi=300, format="pdf")
    plt.show()
